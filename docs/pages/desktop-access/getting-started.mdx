...
<Admonition type="note" title="Secure Cipher Suites">
  ...
  for making TLS connections, so we have to configure a Windows server
  to support those cipher suites as well.
</Admonition>


In this step we'll create a new Certificate Authority's certificate template
with a certificate that uses elliptic curves cryptography and then configure
our GPO to use the newly created template for Remote Desktop connections.
...
3. Find `Computer` template on the list, right-click on it, then select `Duplicate Template`.
4. In the `Compatibility` tab change `Certification Authority` to `Windows Server 2012 R2` and click `OK`.
In the same tab change `Certificate recipient` to `Windows Server 2012 R2` and click `OK`.
5. Go to `General` tab and change `Template display name` to `RemoteDesktopAccess`. Make sure `Template name` is also `RemoteDesktopAccess`.
...
7. Next, in the `Extensions` tab select `Application Polices` and click `Edit` button.
8. Remove all entries from the list.
9. Click `Add…` button, then `New…`. Set the value of `Name` to `Remote Desktop Authentication`. Change the object identifier to `1.3.6.1.4.1.311.54.1.2.` and click `OK`.
10. From the application policies list select `Remote Desktop Authentication` and click `OK`.
11. Go to `Security` tab, select `Domain Controllers` and give the group `Read` and `Enroll` permissions.
12. Finally, create template by clicking `OK`
13. Go back to the Certificate Authority window and right-click on `Certificate Templates`. Then:
```
