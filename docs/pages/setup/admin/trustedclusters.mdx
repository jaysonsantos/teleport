...
$ tctl tokens add --type=trusted_cluster --ttl=5m --labels=env=demo
# Example output:
...
```code
$ tsh login --user=myuser --proxy=cluster.example.com
```
...
```code
$ tsh login --user=myuser --proxy=leaftenant.teleport.sh
```

</ScopedBlock>

To finish establishing trust between your leaf cluster and your root cluster,
create a file called `trusted_cluster.yaml` with the following content:
...
```code
$ curl https://teleport.example.com/webapi/ping | jq .proxy.ssh.tunnel_listen_addr
"0.0.0.0:3024"
```
...
```code
$ curl https://mytenant.teleport.sh/webapi/ping | jq .proxy.ssh.tunnel_listen_addr
"0.0.0.0:3024"
```
...
This field determines the roles that users of the root cluster will acquire when
they log in a Node on the leaf cluster.

For each element in the list of values, `remote` indicates the name of a role on
the root cluster. `local` indicates the names of roles on the leaf cluster. Any
user with the specified role on the root cluster will obtain roles specified in
`local` when logging in to the leaf cluster.
...
```yaml
  role_map:
    - remote: "admin"
      local: ["auditor", "editor"]
```

Any user with the `admin` role on the `root` cluster will be assigned the
`auditor` and `editor` roles when logging in to the leaf cluster.
...
Cluster Name       Status Cluster Type Selected 
------------------ ------ ------------ -------- 
mytenant.teleport.sh online root         *            
```
</ScopedBlock>
