...
they log in to the leaf cluster.
...
user with the specified role on the root cluster will acquire the roles
specified in `local` when logging in to the leaf cluster.
